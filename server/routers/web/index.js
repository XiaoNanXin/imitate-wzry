module.exports = app => {
    ...
    router.get('/news/init',async(req,res) => {
        ...
        //找到所有父级是新闻分类的分类
        let categoriesParent = await Category.find().where({
            parent:parent
        }).lean();
        ...
        let newsList = newsTitles.map(title =>{
            let categoriesList = categoriesParent.slice(0).sort((a,b) => Math.random() - 0.5);
            return{
                categories:categoriesList.slice(0,2),
                title:title
            }
        })
